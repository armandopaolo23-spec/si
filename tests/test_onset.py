...
def recorrer(x, detector=None, marco=MARCO, salto=SALTO):
    """Devuelve la lista de (t, flujo, supera) de recorrer la señal."""
    ...
    for inicio in range(0, len(x) - salto + 1, salto):
        ...
        flujo, supera = detector.procesar(buffer)
        filas.append(((inicio + salto) / SR, flujo, supera))
    return filas
...
def test_primera_ventana_no_dispara():
    ...
    flujo, supera = detector.procesar(np.zeros(MARCO))
...
def test_el_ataque_destaca_sobre_el_sostenido():
    ...
    filas = recorrer(x)
    flujo_ataque = max(f for t, f, _ in filas if 0.19 < t < 0.25)
    flujo_sostenido = max(f for t, f, _ in filas if 0.35 < t < 1.0)
...
def test_el_ruido_de_ambiente_no_supera_el_umbral_ya_calibrado():
    ...
    filas = recorrer(x)
    calibrado = [supera for t, _, supera in filas if t > 0.25]
...
def test_el_piso_decae_y_deja_pasar_la_pulsacion_siguiente():
    ...
    disparos = [t for t, _, supera in recorrer(x, detector) if supera]
